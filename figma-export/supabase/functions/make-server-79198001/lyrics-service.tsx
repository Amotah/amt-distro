...
export async function getLyricsArtist(artistSlug: string, query: LyricsQuery = {}): Promise<{ data: Lyrics[]; count: number }> {
  const supabase = await getSupabaseClient();
  const artistName = decodeURIComponent(artistSlug).replace(/-/g, ' ').trim();
  let q = supabase
    .from('lyrics')
    .select('*', { count: 'exact' })
    .eq('is_published', true)
...
export async function getLyricsAlbum(albumSlug: string, query: LyricsQuery = {}): Promise<{ data: Lyrics[]; count: number }> {
  const supabase = await getSupabaseClient();
  const albumName = decodeURIComponent(albumSlug).replace(/-/g, ' ').trim();
  let q = supabase
    .from('lyrics')
    .select('*', { count: 'exact' })
    .eq('is_published', true)
...
export async function getLyricsByArtistId(artistId: string, query: LyricsQuery = {}): Promise<{ data: Lyrics[]; count: number }> {
  const supabase = await getSupabaseClient();
  let q = supabase
    .from('lyrics')
    .select('*', { count: 'exact' })
  ...
  if (error) {
    console.error('Error fetching lyrics by artist id:', error);
    return { data: [], count: 0 };
  }
